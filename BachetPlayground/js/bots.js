	...
	makeMoveForSituation(gameDataOb){
		...
		for (let i=0; i<this.rules.length; i++){
			if (gameDataOb.N%this.rules[i].a==this.rules[i].b){
				if (gameDataOb.forbiddenMoves.indexOf(this.rules[i].c)==-1){
					res = this.rules[i].c;
					break;				
				}
...
class BotNoRepeatsMinimal extends Bot{
	...
	makeMoveForSituation(gameDataOb){
		let allMoves=this.currentGameRulesObject.allowedMoves.slice()
		let move = allMoves[0]
...
class BotNoRepeatsMaximall extends Bot{
	//об'єкт gameDataOb, який він отримує від світу, мість 2 поля:
	//N - поточну кількість камінців та forbiddenMoves - масив заборонених на даний момент  ходів
	makeMoveForSituation(gameDataOb){
		let allMoves=this.currentGameRulesObject.allowedMoves.slice()
...
class BotNoRepeatsRandom extends Bot{
	//об'єкт gameDataOb, який він отримує від світу, мість 2 поля:
	//N - поточну кількість камінців та forbiddenMoves - масив заборонених на даний момент  ходів
	makeMoveForSituation(gameDataOb){
		let allMoves=this.currentGameRulesObject.allowedMoves.slice()
