class World {
	...
	startTournamentBetweenBots(bots, numGames, numBotsInOneGame=2, showLog=false){
		...
		for (let i=0; i<botsIdsInGames.length; i++){
			...
			//select bots for the current series of games from the general list of bots
...
class UniversalBachetWorld extends BachetWorld{
	...
	validateMove(moveOb){
		let res=false;
		if (moveOb["n"]){
			//if the move made by the player is among the allowed ones
			if (this.allowedMoves.indexOf(moveOb.n)!=-1){
				if (moveOb.n<=this.N){
					res=true;
				}
			}
